...
For More advanced configurations please refer to: DEV/README.md (where the exe exists)
===============================================
""")
...
def remove_share():
    ...
    res = run_ps('Get-SmbShare | Select-Object -ExpandProperty Name')
    shares = [s.strip() for s in res.stdout.splitlines() if s.strip()]
    ...
    run_ps(f'Remove-SmbShare -Name "{choice}" -Force')
